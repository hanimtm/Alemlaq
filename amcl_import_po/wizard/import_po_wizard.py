        ...
        if self and self.file:
            ...
            if self.import_type == 'excel':
                ...
                try:
                    ...
                    created_po  = self.env['purchase.order'].search(
                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if created_po:
                                ...
                                if search_product:
                                    search_product.branch_id = self.branch_id.id
                                    search_product.product_tmpl_id.write({'branch_id':self.branch_id.id})
                                    ...
                                    vals.update({'model_year': search_product.model_year})
                                    vals.update({'grade': search_product.grade})
                                    vals.update({'exterior_color_code': search_product.exterior_color_code})
                                    vals.update({'exterior_color': search_product.exterior_color})
                                    vals.update({'interior_color_code': search_product.interior_color_code})
                                    vals.update({'interior_color': search_product.interior_color})
                                    vals.update({'alj_suffix': search_product.alj_suffix})
                                    vals.update({'vehicle_model': search_product.vehicle_model})
                                    vals.update({'brand': search_product.brand})
                                    vals.update({'sales_document': search_product.sales_document})
                                    vals.update({'item': search_product.item})
